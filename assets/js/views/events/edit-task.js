/* ═══════════════════════════════════════════════════════════════
   YP WORK · VIEWS/EVENTS/EDIT-TASK
   assets/js/views/events/edit-task.js
   ────────────────────────────────────────────────────────────────
   Bottom sheet สำหรับแก้ไข task ในกลุ่มงาน — คล้าย add-task แต่
   pre-fill ค่าเดิม + เรียก store.updateTask() แทน addTask()
   ═══════════════════════════════════════════════════════════════ */
 ...
 * @param {string} taskId — task id ที่จะแก้ไข
 * @param {function} [onUpdated] — เรียกหลังจากอัพเดต task สำเร็จ
 */
export function openEditTaskModal(ev, taskId, onUpdated) {
  const task = ev.tasks?.find(t => t.id === taskId);
  if (!task) {
    toast('ไม่พบ task ที่จะแก้ไข', 'error');
    return;
  }
  ...
  body.innerHTML = `
    ...
    <div class="form-modal__section">
      ...
      <div class="field">
        <input class="input input--lg" id="task-title" required value="${escapeHtml(task.title)}" autocomplete="off" />
    ...
    <div class="form-modal__section">
      ...
      <div class="priority-picker">
        <button type="button" class="priority-option ${task.priority === 'low' ? 'is-selected' : ''}" data-priority="low">
        ...
        <button type="button" class="priority-option ${(!task.priority || task.priority === 'medium') ? 'is-selected' : ''}" data-priority="medium">
        ...
        <button type="button" class="priority-option ${task.priority === 'high' ? 'is-selected' : ''}" data-priority="high">
    ...
    <div class="form-modal__section">
      ...
      <div class="field">
        ...
        <select class="select" id="task-assignee">
          ...
          ${users.map(u => `<option value="${u.id}" ${u.id === task.assigneeId ? 'selected' : ''}>${escapeHtml(u.name)} · ${escapeHtml(u.roleLabel)}</option>`).join('')}
        </select>
      </div>
      <div class="field">
        ...
        <input class="input" type="date" id="task-due" value="${task.dueDate || ''}" />
      </div>
      <div class="field">
        ...
        <input class="input" id="task-est" value="${escapeHtml(task.estimatedTime || '')}" placeholder="เช่น 2 ชม. · 1 วัน · 30 นาที" autocomplete="off" />
      </div>
    </div>
    ...
    <div class="form-modal__section">
      ...
      <div class="field">
        <input class="input" id="task-tags" value="${escapeHtml(tagsStr)}" placeholder="เช่น ด้านเอกสาร, ด้านสถานที่, ด้านการเงิน" autocomplete="off" />
    ...
    <div class="form-modal__section">
      ...
      <div class="field">
        <textarea class="textarea" id="task-notes" placeholder="ขอบเขต · ไฟล์แนบ · ลิงก์อ้างอิง · ฯลฯ" style="min-height:84px;">${escapeHtml(task.notes || '')}</textarea>
      </div>
    </div>
  `;
  ...
  footer.innerHTML = `
    ...
    <button type="button" class="btn btn--primary btn--block" data-submit>${ICON.check || ''} บันทึกการแก้ไข</button>
  `;

  const modal = openModal({ title: 'แก้ไข task', body, footer });
